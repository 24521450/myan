"""P7 Redundant Sense Trim -- guarded apply.

Reads:
  - `data/redundant_sense_trim_p7_decisions.jsonl` (59 P7 decisions)

Writes (with --apply; otherwise dry-run):
  - `data/audit_full_deck_v2.jsonl` (2487 rows; exactly 59 updated)
  - `English Academic Vocabulary.txt` (cells updated for matching rows)
  - (Rebuild JSONL via `tools/build_notes.py` after apply.)

Guardrails (per P7 plan):
  - Match audit rows by 5-element key
    `(word, pos, cefr, def_before, old_gloss)`.
  - Exactly 59 audit rows changed.
  - All 59 decisions' `new_gloss` passes `validate_verdict`.
  - All 59 decisions' `gloss_word_count` matches actual count.
  - rule_after is `common_core_trimmed` (single-chunk) or
    `trimmed_multisense` (multi-chunk); no v3 rule labels carried over.
  - fix_status is `p7_redundant_sense_trimmed`.
  - Audit row count preserved at 2487.

Run:
  python -m tools._apply_p7_redundant_sense_trim            # dry-run (default)
  python -m tools._apply_p7_redundant_sense_trim --apply    # write
"""
...
def main() -> int:
    ...
    print('=' * 72)
    print(f'P7 Redundant Sense Trim Apply (apply={args.apply})')
    ...
    if len(decisions) != 59:
        print(f'FATAL: decisions has {len(decisions)} rows (expected 59)')
        return 1
    ...
    for d in decisions:
        ...
        if rule_after not in ('common_core_trimmed', 'trimmed_multisense'):
            failures.append(
        ...
        if fix_status != 'p7_redundant_sense_trimmed':
            failures.append(
                f'  ({d["word"]}, {d["pos"]}, {d["cefr"]}) fix_status={fix_status!r} '
    ...
    for r in audit_rows:
        ...
        replaced += 1
    if replaced != 59:
        print(f'FATAL: replaced {replaced} audit rows (expected 59)')
        return 1
    ...
    # === Apply ===
    print('\n[6] Writing changes...')
    audit_bak = AUDIT_PATH.with_suffix(AUDIT_PATH.suffix + f'.bak_pre_p7_redundant_sense_trim_{_ts()}')
    txt_bak = TXT_PATH.with_suffix(TXT_PATH.suffix + f'.bak_pre_p7_redundant_sense_trim_{_ts()}')
